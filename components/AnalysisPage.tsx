...
const AnalysisPage: React.FC<AnalysisPageProps> = ({ data }) => {
  ...
  const benchmarkData = useMemo(() => {
    return companies.map(c => {
      ...
      if (benchMetric === 'yield') {
        if (benchYear === 'Current') {
          val = (c as any).dividendYieldNumeric || 0;
        } else if (benchYear === 'Forward') {
          val = (c as any).dividendYield2025Numeric || 0;
        } else { // 'Next'
          val = (c as any).dividendYield2026Numeric || 0;
        }
      } else {
  ...
  const handleAIQuery = useCallback(async (e: React.FormEvent) => {
    ...
    setAiResponse(null);
    try {
      const response = await queryCompanyAI(query, data);
      setAiResponse(response);
    } catch (err) {
      setAiResponse("Le service d'analyse est momentanément indisponible. Veuillez réessayer.");
    } finally {
    ...
    <div className="animate-in fade-in duration-700 max-w-6xl mx-auto pb-24 space-y-8 px-2">
      <div className="bg-slate-900/40 rounded-[2.5rem] p-6 md:p-10 border border-slate-800 shadow-2xl backdrop-blur-md">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          <div><h2 className="text-xl font-bold text-white uppercase tracking-tight">Performance Relative</h2><p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-1">Base 0 au début de période</p></div>
        ...
        <div className="h-[250px] md:h-[350px] w-full relative mb-12">
          ...
          {!isChartInitiated && !loadingHistory && (
            <div className="absolute inset-0 flex items-center justify-center z-20">
              <button
                onClick={() => setIsChartInitiated(true)}
                className="bg-emerald-600 hover:bg-emerald-500 text-white font-black text-sm uppercase tracking-widest px-8 py-5 rounded-2xl shadow-lg transition-all transform hover:scale-105"
              >
      ...
      <div className="bg-[#0f172a] rounded-[3rem] p-8 md:p-14 border border-slate-800 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16"><div><h2 className="text-2xl font-black text-white uppercase tracking-tight">Comparatif Sectoriel</h2><p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-1">Analyse des multiples de valorisation</p></div><div className="flex flex-wrap gap-3"><select value={benchMetric} onChange={e => setBenchMetric(e.target.value as MetricKey)} className="bg-slate-900 border border-slate-700 text-emerald-400 text-[10px] font-black uppercase p-4 rounded-2xl outline-none hover:border-emerald-500/50 transition-colors shadow-lg"><option value="evEbitda">EV/EBITDA</option><option value="evEbit">EV/EBIT</option><option value="evEbitdaCapex">EV/(EBITDA-CAPEX)</option><option value="evSales">EV/Sales</option><option value="per">P/E</option><option value="yield">Dividend Yield</option></select><select value={benchYear} onChange={e => setBenchYear(e.target.value as YearKey)} className="bg-slate-900 border border-slate-700 text-emerald-400 text-[10px] font-black uppercase p-4 rounded-2xl outline-none hover:border-emerald-500/50 transition-colors shadow-lg"><option value="Current">2024</option><option value="Forward">2025E</option><option value="Next">2026E</option></select></div></div>
        <div className="text-center mb-16 animate-in fade-in slide-in-from-top-4 duration-500"><div className="text-5xl md:text-8xl font-black text-white tracking-tighter tabular-nums drop-shadow-2xl">{displayBench?.value.toFixed(1)}<span className="text-xl md:text-2xl text-emerald-500 ml-4 font-black uppercase tracking-widest">{benchMetric === 'yield' ? '%' : 'x'}</span></div><div className="mt-4 flex flex-col items-center"><span className="text-sm font-black text-emerald-400 uppercase tracking-[0.3em]">{displayBench?.ticker}</span><span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1 opacity-60">{displayBench?.name}</span></div></div>
        <div className="relative h-48 md:h-64 flex items-end gap-3 md:gap-5 border-b border-slate-800 px-6 pb-2">{benchmarkData.map(d => { const h = (d.value / maxBenchValue) * 100; const isActive = displayBench?.ticker === d.ticker; return (<div key={d.ticker} className="flex-1 flex flex-col items-center group relative h-full justify-end cursor-pointer" onMouseEnter={() => setActiveBenchTicker(d.ticker)}><div className={`mb-2 text-[10px] font-black transition-all duration-300 ${isActive ? 'text-emerald-400 scale-110' : 'text-slate-500 opacity-60'}`}>{d.value.toFixed(1)}{benchMetric === 'yield' ? '%' : 'x'}</div><div className={`w-full rounded-t-2xl transition-all duration-500 relative ${isActive ? 'bg-gradient-to-t from-emerald-600 to-emerald-400 shadow-[0_-10px_30px_rgba(16,185,129,0.3)]' : 'bg-slate-800/60 group-hover:bg-slate-700 border-x border-t border-slate-700/50'}`} style={{ height: `${Math.max(8, h)}%` }}>{isActive && <div className="absolute inset-0 bg-white/20 animate-pulse rounded-t-2xl"></div>}</div><div className={`absolute top-full mt-4 text-[9px] font-black transition-colors ${isActive ? 'text-emerald-400' : 'text-slate-600 group-hover:text-slate-400'}`}>{d.ticker}</div></div>);})}</div>
      </div>
      <section className="bg-slate-900/40 p-8 rounded-[2.5rem] border border-slate-800 shadow-xl backdrop-blur-sm">
        <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-white uppercase tracking-tight">Analyse Stratégique</h3>
            <AIBadge />
        </div>
        <form onSubmit={handleAIQuery} className="flex gap-3 mb-6">
          <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Comparer la valorisation de JCDecaux vs Ströer..." className="flex-1 bg-slate-800/50 border border-slate-700 rounded-2xl px-6 py-4 outline-none text-white text-sm focus:ring-2 focus:ring-emerald-500/50 transition-all" />
          <button type="submit" disabled={isAiLoading} className="bg-emerald-600 px-8 py-4 rounded-2xl font-black text-[10px] uppercase hover:bg-emerald-500 disabled:opacity-50 text-white transition-colors">
            {isAiLoading ? "..." : "Analyser"}
          </button>
        </form>
        {isAiLoading && <div className="p-6 text-center text-slate-500 text-sm">Analyse en cours...</div>}
