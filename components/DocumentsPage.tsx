...
// FIX: Moved DocumentCard outside the DocumentsPage component to prevent re-rendering and fix typing issues.
const DocumentCard: React.FC<DocumentCardProps> = ({ doc, ticker }) => {
  ...
  return (
    ...
    >
      ...
      <div className="mt-auto pt-6 border-t border-slate-700/50 flex items-center justify-between">
        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter">Source: {ticker}</span>
        {hasUrl ? (
      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {currentDocs.length > 0 ? (
            // FIX: Use stable doc.id for key and pass selectedTicker to DocumentCard.
            currentDocs.map((doc) => (
              <DocumentCard key={doc.id} doc={doc} ticker={selectedTicker} />
            ))
          ) : (
