...
const NewsPage: React.FC<NewsPageProps> = ({ news, highlights, sentiment, loading }) => {
  ...
  return (
    <div className="space-y-6 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-5xl mx-auto pb-20 md:pb-0">
      
      <section className="bg-slate-800/60 p-3 md:p-6 rounded-lg md:rounded-2xl border border-slate-700/80 shadow-xl">
      ...
      <section className="space-y-4 md:space-y-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-6 px-1">
          ...
          <div className="flex bg-slate-800 p-1 rounded-xl border border-slate-700 overflow-x-auto max-w-full no-scrollbar shadow-lg">
            {tags.map(tag => (
            ...
            filteredNews.map((item, idx) => (
              <button key={idx} onClick={() => setSelectedNews(item)} className={`text-left bg-slate-800 p-3 md:p-6 rounded-lg md:rounded-[2rem] border transition-all flex flex-col md:flex-row justify-between items-start md:items-center group gap-4 md:gap-6 shadow-xl w-full ${item.tag?.toLowerCase() === 'deals' ? 'border-amber-500/20 bg-gradient-to-br from-slate-800 to-amber-900/10 hover:border-amber-500/50' : 'border-slate-700 hover:border-emerald-500/50'}`}>
                <div className="flex-1">
                  ...
                  <h3 className="text-sm md:text-xl font-bold text-slate-100 group-hover:text-emerald-400 transition-colors leading-snug">{item.title}</h3>
                </div>
      ...
      <Modal isOpen={!!selectedNews} onClose={closeOverlay}>
        {selectedNews && (
          <div className="bg-slate-800 w-full max-w-lg md:max-w-2xl rounded-xl md:rounded-3xl border border-slate-700 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-4 md:p-10 border-b border-slate-700/50 flex justify-between items-start gap-4">
            ...
            <div className="p-4 md:p-10 space-y-6 max-h-[60vh] overflow-y-auto no-scrollbar">
            ...
            <div className="p-4 md:p-6 bg-slate-900/30 border-t border-slate-700/50 flex justify-center"><button onClick={closeOverlay} className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] hover:text-white transition-colors">[ Retour au flux ]</button></div>
          </div>
        )}
      </Modal>
    </div>
