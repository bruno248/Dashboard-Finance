...
export const MarketTable: React.FC<MarketTableProps> = ({ companies, onSelectCompany }) => {
  ...
  return (
    <div className="bg-slate-800 rounded-xl md:rounded-2xl border border-slate-700 overflow-hidden shadow-xl">
      <div className="overflow-x-auto no-scrollbar">
        <table className="w-full text-left text-sm md:min-w-[600px]">
          <thead className="bg-slate-800/50 border-b border-slate-700">
            <tr>
              <th className="px-2 py-2 md:px-6 md:py-4 font-semibold text-slate-400 text-[9px] md:text-[10px] uppercase tracking-widest">Valeur</th>
              <th className="px-2 py-2 md:px-6 md:py-4 font-semibold text-slate-400 text-[9px] md:text-[10px] uppercase tracking-widest text-right">Cours (LC)</th>
              <th className="hidden sm:table-cell px-2 py-2 md:px-6 md:py-4 font-semibold text-slate-400 text-[9px] md:text-[10px] uppercase tracking-widest text-right">Objectif (LC)</th>
              <th className="px-2 py-2 md:px-6 md:py-4 font-semibold text-slate-400 text-[9px] md:text-[10px] uppercase tracking-widest text-center">Var. %</th>
              ...
              <th className="px-2 py-2 md:px-6 md:py-4 font-semibold text-slate-400 text-[9px] md:text-[10px] uppercase tracking-widest text-right">Rating</th>
            </tr>
          </thead>
              ...
              <tr key={company.id} className="hover:bg-slate-700/30 transition-colors duration-200 ease-in-out cursor-pointer group" onClick={() => onSelectCompany(company)}>
                <td className="px-2 py-2 md:px-6 md:py-4">
                  <div className="flex items-center gap-2 md:gap-3">
                    <div className="w-7 h-7 md:w-8 md:h-8 rounded bg-slate-700 flex items-center justify-center font-bold text-[9px] md:text-[10px] text-emerald-400 shadow-inner flex-shrink-0">
                ...
                <td className="px-2 py-2 md:px-6 md:py-4 font-bold text-slate-100 text-right text-xs md:text-sm whitespace-nowrap">
                  {(company.price || 0).toFixed(2)}
                </td>
                <td className="hidden sm:table-cell px-2 py-2 md:px-6 md:py-4 font-bold text-emerald-400 text-right text-xs md:text-sm whitespace-nowrap">
                  {company.targetPrice ? company.targetPrice.toFixed(2) : '--'}
                </td>
                <td className={`px-2 py-2 md:px-6 md:py-4 font-black text-center text-xs md:text-sm ${(company.change || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
